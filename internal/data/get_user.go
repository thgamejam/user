...
import "user/internal/biz"

import (
	"context"
	userV1 "user/proto/api/user/v1"
)
...
func (r *userRepo) GetUserAvatarURL(ctx context.Context, avatarID string) (string, error) {
	...
	// 缓存中不存在则从对象存储里获取URL
	url, err := r.data.ObjectStorage.PreSignGetURL(ctx, r.data.UserConf.UserAvatarBucketName, avatarID, avatarID, -1)
	if err != nil {
		// 若对象存储报错则尝试获取默认头像
		url, err = r.data.ObjectStorage.PreSignGetURL(ctx, r.data.UserConf.UserAvatarBucketName, r.data.UserConf.DefaultUserAvatarHash, r.data.UserConf.DefaultUserAvatarHash, -1)
